...
uint32_t wayland_wl_registry_bind(int fd, uint32_t registry,
...
// @wayland_wl_compositor_create_surface
...
// @wayland_xdg_surface_ack_configure
void wayland_xdg_surface_ack_configure(int fd, state_t *state,
                                       uint32_t configure) {
...
// @wayland_xdg_wm_base_get_xdg_surface
...
void wayland_handle_message(int fd, state_t *state, char **msg,
                            uint64_t *msg_len) {
    assert(*msg_len >= 8);
    ...
    if (object_id == state->wl_registry &&
        ...
        if (strcmp(wl_shm_interface, interface) == 0) {
            state->wl_shm = wayland_wl_registry_bind(
                fd, state->wl_registry, name, interface, interface_len, (version > 1 ? 1 : version));
        }
        ...
        uint32_t payload      = announced_size - header_size;
        printf("<- xdg_toplevel@%u.event %u  (skipping %u bytes)\n",
               state->xdg_toplevel, opcode, payload);
        *msg     += payload;
        *msg_len -= payload;
        return;
    } else if (object_id == state->wl_surface) {
        uint32_t payload     = announced_size - header_size;
        printf("<- wl_surface@%u.event %u (skipping %u bytes)\n",
               state->wl_surface, opcode, payload);
        *msg     += payload;
        *msg_len -= payload;
        return;
    }
